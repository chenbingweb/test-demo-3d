...
/*
 ...
 SelfTranslation_SelfRotate() {
    let v1 = new Vector3();
    this.entity.transform.getWorldForward(v1);
    let v2 = new Vector3();
    this.cameraNode.transform.getWorldForward(v2);
    let worldOffset = v2.subtract(v1);
    let itemBaseVec = this.entity.transform.localMatrix;
    console.log(itemBaseVec);
    let { xAxis, yAxis, zAxis } = this.extractBasis(itemBaseVec.clone());
    let localOffset = new Vector3();
    localOffset.x = this.dot2(worldOffset, xAxis);
    localOffset.y = this.dot2(worldOffset, yAxis);
    localOffset.z = this.dot2(worldOffset, zAxis);
    console.log(localOffset);
    let goZeroMatrix = new Matrix().translate(localOffset);
    this.cameraNode.transform.localMatrix.multiply(goZeroMatrix);
    let angle = (30 * Math.PI) / 180;
    let absAngle = Math.abs(angle);
    this.cameraNode.transform.localMatrix.multiply(
      new Matrix().rotateAxisAngle(new Vector3(0, 1, 9), absAngle)
    );

    let mat4I = new Matrix();
    mat4I.copyFrom(goZeroMatrix).invert();
    this.cameraNode.transform.localMatrix.multiply(mat4I);
    // debugger;
    // itemBaseVec.transpose
    // worldOffset.multiply(itemBaseVec.x)
    // let localOffset =
  }
  dot2(v1: Vector3, v2: Vector3) {
    return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
  }
  setFromMatrixColumn(m: Matrix, index: number) {
    return this.fromArray(m.elements, index * 4);
  }
  fromArray(array: ArrayBuffer, offset: number) {
  ...
  extractBasis(m: Matrix) {
    let xAxis = this.setFromMatrixColumn(m, 0);
    let yAxis = this.setFromMatrixColumn(m, 1);
    let zAxis = this.setFromMatrixColumn(m, 2);
    return {
      xAxis: xAxis.clone(),
      yAxis: yAxis.clone(),
      zAxis: zAxis.clone(),
